...
class InputEncoder(abc.ABC, Generic[InputT, OutputT]):
    ...
    def encode(self, value: InputT) -> OutputT:
        raise NotImplementedError('"encode" method has not been implemented for this type.')

    @property
    @abc.abstractmethod
    def output_dim(self) -> int:
        raise NotImplementedError('"output_dim" method has not been implemented for this type.')
...
class InputEncoderNormalizedRange(InputEncoder[float, Tensor]):
    ...
    def encode(self, value: InputT) -> OutputT:
        if not (self._in_min <= value <= self._in_max):
            raise ValueError(f'Value ({value}) is out of range ({self._in_min}, {self._in_max})')

        value_norm = (value - self._in_min) / (self._in_max - self._in_min)
        value_norm = value_norm * (self._out_max - self._out_min) + self._out_min
        value_norm = torch.tensor(value_norm, dtype=self._dtype)
        return value_norm
